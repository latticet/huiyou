...
$api->post('/login', function (Request $request) {
    ...
    $expiredTime=time()+60*60*24*30*6;
    $token = ServiceKernel::instance()->createService('User.UserService')->makeToken('mobile_login', $user['id'],$expiredTime);
    setCurrentUser($user);
...
$api->post('/reset_password/{mobileNumber}', function (Request $request, $mobileNumber) {
    ...
    $user = $UserService->getUserByLoginField($mobileNumber);
