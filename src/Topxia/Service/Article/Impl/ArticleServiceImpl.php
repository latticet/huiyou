...
class ArticleServiceImpl extends BaseService implements ArticleService {
    ...
    public function findArticlesByIdsAndCatId($ids, $categoryId) {
        ...
        return ArrayToolkit::index($this->getArticleDao()->findArticlesByIdsAndCatId($ids, $categoryId) , 'id');
    }
    ...
    public function searchArticleLikesByUserId($userId,$start,$limit) {
        ...
        return $this->getArticleLikeDao()->searchArticleLikesByUserId($userId,$start,$limit);
    }
    public function getArticleLikesCountByArticleId($articleId) {
        
        return $this->getArticleLikeDao()->getArticleLikesCountByArticleId($articleId);
    }
    ...
    public function setArticleProperty($id, $property) {
        ...
        $propertyVal = 1;
        $this->getArticleDao()->updateArticle($id, array(
    ...
    public function trashArticle($id) {
        ...
        $this->getArticleDao()->updateArticle($id, $fields = array(
            'status' => 'trash'
        ));
    ...
    public function removeArticlethumb($id, $target = '') {
        ...
        if (!empty($target)) {
            $thumb = $target . '_thumb';
            $originalThumb = $target . '_originalThumb';
        } else {
            $thumb = 'thumb';
    ...
    protected function filterArticleFields($fields, $mode = 'update') {
        ...
        //如果是活动
        if (isset($fields['is_activity']) && $fields['is_activity'] == '1') {
            $article['is_activity'] = $fields['is_activity'];
            $article['max'] = $fields['max'];
            $article['organizer'] = $fields['organizer'];
            $article['activity_start'] = strtotime($fields['activity_start']);
            $article['activity_end'] = strtotime($fields['activity_end']);
            $article['suitable'] = $fields['suitable'];
            $article['activity_type'] = $fields['activity_type'];
            $article['detail_thumb'] = $fields['detail_thumb'];
            $article['detail_originalThumb'] = $fields['detail_originalThumb'];
            $article['carousel01_thumb'] = $fields['carousel01_thumb'];
            $article['carousel01_originalThumb'] = $fields['carousel01_originalThumb'];
            $article['carousel02_thumb'] = $fields['carousel02_thumb'];
            $article['carousel02_originalThumb'] = $fields['carousel02_originalThumb'];
            $article['carousel03_thumb'] = $fields['carousel03_thumb'];
            $article['carousel03_originalThumb'] = $fields['carousel03_originalThumb'];
        }
    ...
    protected function prepareSearchConditions($conditions) {
        ...
        if (!empty($conditions['includeChildren']) && isset($conditions['categoryId'])) {
            ...
            $conditions['categoryIds'] = array_merge(array(
