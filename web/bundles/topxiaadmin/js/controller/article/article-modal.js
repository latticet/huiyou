define(function(require, exports, module) {
    ...
    function _initEditorFields($form, validator) {
        ...
        $("#article_detail_thumb_remove").on('click', function(){
            if (!confirm('确认要删除吗？')) return false;
            var $btn = $(this);
            $.post($btn.data('url'), function(){
                $("#article-detail_thumb-container").html('');
                $form.find('[name=detail_thumb]').val('');
                $form.find('[name=detail_originalThumb]').val('');
                $btn.hide();
                Notify.success('删除成功！');
            }).error(function(){
                Notify.danger('删除失败！');
        ...
        $("#article_carousel01_thumb_remove").on('click', function(){
            if (!confirm('确认要删除吗？')) return false;
            var $btn = $(this);
            $.post($btn.data('url'), function(){
                $("#article-carousel01_thumb-container").html('');
                $form.find('[name=carousel01_thumb]').val('');
                $form.find('[name=carousel01_originalThumb]').val('');
                $btn.hide();
                Notify.success('删除成功！');
            }).error(function(){
                Notify.danger('删除失败！');
        ...
        $("#article_carousel02_thumb_remove").on('click', function(){
            if (!confirm('确认要删除吗？')) return false;
            var $btn = $(this);
            $.post($btn.data('url'), function(){
                $("#article-carousel02_thumb-container").html('');
                $form.find('[name=carousel02_thumb]').val('');
                $form.find('[name=carousel02_originalThumb]').val('');
                $btn.hide();
                Notify.success('删除成功！');
            }).error(function(){
                Notify.danger('删除失败！');
        ...
        $("#article_carousel03_thumb_remove").on('click', function(){
            if (!confirm('确认要删除吗？')) return false;
            var $btn = $(this);
            $.post($btn.data('url'), function(){
                $("#article-carousel03_thumb-container").html('');
                $form.find('[name=carousel03_thumb]').val('');
                $form.find('[name=carousel03_originalThumb]').val('');
                $btn.hide();
                Notify.success('删除成功！');
            }).error(function(){
                Notify.danger('删除失败！');
